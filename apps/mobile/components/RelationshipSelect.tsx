 ...
 *
 * ตัวเลือกความสัมพันธ์กับผู้สูงอายุแบบ bottom sheet
 *
 * สิ่งที่เกิดขึ้นในไฟล์นี้:
 * - แสดงช่องเลือกความสัมพันธ์ด้วย FloatingLabelInput
 * - เปิด bottom sheet ให้ผู้ใช้เลือกความสัมพันธ์จากค่าที่กำหนดไว้
...
import { EMERGENCY_RELATIONSHIP_OPTIONS } from '../utils/emergencyRelationship';
...
const SHEET_HEIGHT = 56 + 56 * EMERGENCY_RELATIONSHIP_OPTIONS.length + 32;

export function RelationshipSelect({
  value,
  onChange,
  label = 'ความสัมพันธ์กับผู้สูงอายุ',
  error,
}: RelationshipSelectProps) {
  const MODAL_KEY = 'relationship-select';
  ...
  const getDisplayValue = (currentValue: string) =>
    EMERGENCY_RELATIONSHIP_OPTIONS.find((option) => option.value === currentValue)?.label ?? '';
        ...
        <View style={StyleSheet.absoluteFill}>
          <View style={styles.container}>
            ...
            <Animated.View style={[styles.sheet, { transform: [{ translateY }] }]}>
              ...
              {EMERGENCY_RELATIONSHIP_OPTIONS.map((option, idx) => {
                const isSelected = value === option.value;
                const isLast = idx === EMERGENCY_RELATIONSHIP_OPTIONS.length - 1;
